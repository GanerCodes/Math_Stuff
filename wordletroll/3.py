...
U = fs({ sum (sorted(w)) for w in W if w and  len (fs(w))== len (w)})

class  MAN:
    def  __init__( SPECIAL_SELF_ , U):
        SPECIAL_SELF_. U, SPECIAL_SELF_. C = U,{}
        for w in U:
            for l in w:
                if l in  SPECIAL_SELF_. C: SPECIAL_SELF_. C[l]|={w}
                else : SPECIAL_SELF_. C[l]={w}
        SPECIAL_SELF_. O = sorted( SPECIAL_SELF_. U, key= SPECIAL_SELF_. cac)
    __call__ = SPECIAL_SELF_ ,w$> SPECIAL_SELF_. U-set.union(*( SPECIAL_SELF_. C[l] for l in w))
    cac = SPECIAL_SELF_ ,st$> sum ( len ( SPECIAL_SELF_. C[l]) for l in st)
    first = SPECIAL_SELF_ $>{ SPECIAL_SELF_. O[0]} if  SPECIAL_SELF_. O else  None 

def  K_N(g, n, I):
    if n==1:return g.first()
    if  len (g.O)<n:return 
    for v in g.O:
        if (G:=K_N(MAN(g(v)),n-1,I)) and (((s:={v}.union(G))-I) or  len (s)!=5): return s
mp = { sum (sorted(w)):w for w in W}

S,I = U, set() 
while  True :
    print * len (set( sum ( print *{mp[ sum (sorted(w))] for w in (m:=K_N(MAN(S),5,I))})))
    I = I.union(m)