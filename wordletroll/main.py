...
U = [w for w in W if w and  len (set(w))== len (w)]
...
while  True :
    ...
    for x in  range *5:
        F = freq(t)
        wrds = {w:word_diff(w,F) for w in t}
        wrds_by_diff = sorted(wrds.items(),key=<$x$>x[1])
        if  not  len (wrds_by_diff): break 
        WORDS+=[W:=choice(wrds_by_diff[:(x+2)**2])[0]]
        # N = choice( range *2)
        t = [w for w in t if  all (l not in W for l in w)]
    
    print (*((k:= sum (sorted( list *set( sum (WORDS)))))`$ len (k))[::-1], WORDS)
